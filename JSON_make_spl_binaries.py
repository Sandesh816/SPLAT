...
The binary combinations data is stored in JSON format for the SpexBinaryFit.py program.
...
# Add the following bottom 4 lines only if you want to make binaries of specific spectrum type objects
valid_values =  ['L3.0', 'L4.0', 'L5.0']
mask = specdf['spt'].isin(valid_values)
specdf = specdf[mask]
print("The new length is ", len(specdf['spt']))

# Add the following bottom 1 line only if you want to make combinations of limited number of objects
spl = specdf['sp'][:10]
spl = specdf['sp']
